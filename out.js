...
Case9.main = function() {
	var datas = [];
	var pip = datas;
	var ires = _$Pipeline_Pipeline_$Impl_$.select(_$Pipeline_Pipeline_$Impl_$.where(_$Pipeline_Pipeline_$Impl_$.where(pip,function(x) {
		return x.age >= 20;
	}),function(x1) {
		return x1.name.charAt("n");
	}),function(x2) {
		return x2.name;
	});
	...
	var res = $iterator(_$Pipeline_Pipeline_$Impl_$)(ires);
	if(!(res.hasNext() && res.next() == "tom")) {
		console.log("failed");
	}
	if(!(res.hasNext() && res.next() == "alice")) {
		console.log("failed");
	}
	if(!(!res.hasNext())) {
		console.log("failed");
	...
	res = $iterator(_$Pipeline_Pipeline_$Impl_$)(ires);
	if(!(res.hasNext() && res.next() == "tom")) {
		console.log("failed");
	}
	if(!(res.hasNext() && res.next() == "alice")) {
