...
(function () { "use strict";
var Case16 = function() { };
Case16.main = function() {
	var tar = { x : "X", y : "Y"};
	var ans = { x : "X", y : "Y"};
	var pip = _$Pipeline_Pipeline_$Impl_$.openFields(tar);
	var itr = $iterator(_$Pipeline_Pipeline_$Impl_$)(pip);
	var _g = 0;
	var _g1 = Reflect.fields(ans);
	while(_g < _g1.length) {
		var x = _g1[_g];
		++_g;
		if(!itr.hasNext()) {
		...
		var kv = itr.next();
		if(!(x == kv.key && Reflect.field(ans,x) == kv.value)) {
			console.log("failed");
		}
	}
	if(!(!itr.hasNext())) {
