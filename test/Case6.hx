...
{id:99,name:"xkjydkb",age:69,male:true}
		...
		var min=10;
		var max=69;
		if(!(dat.aggregate(0,function(base,x){return base+x.age;})==sum))trace("failed");
		if(!(Math.abs(
				dat
				.aggregate(
					0.0,
					function(base:Float,x:Dynamic):Float{return base+x.age;},
					function(base:Float,count:Int):Float{return base/count;}
					)
				-average
				)<=1e-9))trace("failed");
		if(!(dat.argmin(function(x){return x.age;}).age==min))trace("failed");
		if(!(dat.argmax(function(x){return x.age;}).age==max))trace("failed");
