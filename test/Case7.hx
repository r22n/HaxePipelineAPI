...
{id:99,name:"xkjydkb",age:69,male:true}
		...
		var first="pqiyqcd";
		var last="xkjydkb";
		if(!(dat.where(function(x){return x.male;}).count()==count))trace("failed");
		if(!(dat.where(function(x){return x.male;}).select(function(x){return x.name;}).first()==first))trace("failed");
		if(!(dat.where(function(x){return x.male;}).select(function(x){return x.name;}).last()==last))trace("failed");
